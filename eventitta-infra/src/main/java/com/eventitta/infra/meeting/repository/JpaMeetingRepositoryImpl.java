package com.eventitta.infra.meeting.repository;

import com.eventitta.domain.meeting.domain.MeetingStatus;
import com.eventitta.domain.meeting.domain.QMeeting;
...
import com.eventitta.domain.meeting.repository.MeetingRepositoryCustom;
...
@Repository
public class JpaMeetingRepositoryImpl implements MeetingRepositoryCustom {
    ...
    public Page<MeetingSummaryResponse> findMeetingsByFilter(MeetingFilter filter, Pageable pageable) {
        ...
        if (hasDistanceFilter) {
            ...
            contentQ = qf.select(Projections.constructor(
                    ...
                    m.status,
                    m.leaderId, u.nickname,
                    distance
                ))
                .from(m)
                .leftJoin(u).on(m.leaderId.eq(u.id))
                .where(where)
